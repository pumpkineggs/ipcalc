...
if mask_full_octets > 0:
    ...
    if mask_remain_bits > 0:
        ...
        # wildcard calculation - it's inverted mask
        wildcard_str=''
        for bit in mask_remain_octet_str:
            if bit == "1":
                wildcard_str += "0"
            else:
                wildcard_str += "1"
        print 'Wildcard binary', wildcard_str #wildcard_decimal[remain_octet_index] = mask_remain_octet
        print 'New mask: ', '.'.join(str(e) for e in mask_decimal)
        #print 'Wildcard: ', '.'.join(str(e) for e in wildcard_decimal)
    else:
